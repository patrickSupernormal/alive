---
description: "The human wants to adjust how the system behaves — not what it contains, but how it feels. Voice, rhythm, preferences, walnut-level config. The system adapting to them. Routes to preferences.yaml or walnut config.yaml depending on scope. For creating new skills, rules, or hooks, route to alive:extend instead."
user-invocable: true
---
...
For creating new skills, rules, and hooks — that's `alive:extend`.
...
**The line:** Toggle = preference. Setting = config. Process or capability = `alive:extend`.
...
3. **Is it a repeatable process or new capability?** → Route to `alive:extend`.
...
External context the system knows about. Used by `alive:world` (dashboard), `alive:history` (session timeline), `alive:find` (query), and `alive:capture` (import).
...
│
│  Recommendation: run alive:tidy to address 27 incomplete capsules
│  and stale drafts.
╰─
```
...
Draft an adapter concept, then route to `alive:extend` for implementation. MCP integration where possible, manual import flow where not.
