...
Build custom capabilities that integrate cleanly with ALIVE.

Not about adjusting preferences or voice (that's `alive:tune`). Extend is about creating NEW things — skills, rules, hooks — that make the system do something it couldn't before.
...
│  Fires: after every email capture via alive:capture
...
- Not `alive:tune` — tune adjusts preferences and config. Extend creates new capabilities.
