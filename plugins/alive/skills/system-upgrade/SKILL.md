...
11. **Regenerate all projections** -- run `project.py --walnut {path}` for EVERY walnut to generate fresh `_kernel/now.json`. Then run `generate-index.py` on the world root to rebuild the world index with task counts, recent sessions, and unsigned stash data. This is the final step that makes `/alive:world` work.
