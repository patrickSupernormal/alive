...
- `alive:history` escalates — "these sessions look rich, want to deep-mine them?"
...
## Mining Sessions (via alive:history)

When `alive:history` identifies unmined sessions — long transcripts with extensive decision-making or research — it can hand off to mine with a specific scope:
...
The squirrel resolves transcript paths using the same discovery mechanism as history (see `alive:history` Transcript Discovery section).
...
- Not `alive:capture` — capture brings new content IN. Mine extracts value from content already captured.
- Not `alive:history` — history shows the session timeline. Mine goes deep on specific sources.
- Not `alive:find` — find searches for known things. Mine discovers unknown things.
